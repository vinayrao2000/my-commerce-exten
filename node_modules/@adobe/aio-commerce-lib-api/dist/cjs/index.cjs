...
//#endregion
const require_hooks = require('./hooks-CZhnmKy0.cjs');
...
*/
function optionallyExtendKy(kyInstance, options) {
	return options ? kyInstance.extend(options) : kyInstance;
}
...
function buildCommerceHttpClientPaaS(params) {
	...
	return optionallyExtendKy(createKy({
		prefixUrl: getCommerceUrl(config),
		hooks: { beforeRequest: [buildAuthBeforeRequestHook("paas", auth)] }
	}), fetchOptions);
}
/**
...
function buildCommerceHttpClientSaaS(params) {
	...
	return optionallyExtendKy(createKy({
		prefixUrl: getCommerceUrl(config),
		hooks: { beforeRequest: [buildAuthBeforeRequestHook("saas", auth), (request) => {
