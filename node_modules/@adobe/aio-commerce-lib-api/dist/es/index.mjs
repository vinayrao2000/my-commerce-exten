...
import { n as buildIntegrationAuthBeforeRequestHook, r as isAuthProvider, t as buildImsAuthBeforeRequestHook } from "./hooks-Dc1jLa88.mjs";
import { forwardImsAuthProvider, isImsAuthProvider, resolveAuthParams } from "@adobe/aio-commerce-lib-auth";
...
const ky = kyModule.default.default ?? kyModule.default;
/**
...
function buildAuthBeforeRequestHook(flavor, auth) {
	if (isImsAuthProvider(auth) || "clientId" in auth) return isAuthProvider(auth) ? buildImsAuthBeforeRequestHook(auth) : buildImsAuthBeforeRequestHook(ensureImsScopes(auth, flavor === "saas" ? COMMERCE_SAAS_IMS_REQUIRED_SCOPES : COMMERCE_IMS_REQUIRED_SCOPES));
...
function resolveCommerceHttpClientParams(params, options = {}) {
	if (allNonEmpty(params, ["AIO_COMMERCE_API_BASE_URL"])) {
		...
		let clientAuth;
		if (tryForwardAuthProvider) clientAuth = forwardImsAuthProvider(params);
		else {
...
function buildIoEventsHttpClient(params) {
	const { auth, config, fetchOptions } = params;
	const beforeRequestAuthHook = isAuthProvider(auth) ? buildImsAuthBeforeRequestHook(auth) : buildImsAuthBeforeRequestHook(ensureImsScopes(auth, IO_EVENTS_IMS_REQUIRED_SCOPES));
...
//#endregion
export { AdobeCommerceHttpClient, AdobeIoEventsHttpClient, ApiClient, resolveCommerceHttpClientParams, resolveIoEventsHttpClientParams };