...
function buildActionDefinition(action, existingAction) {
	...
	if (includes.length > 0) {
		const itemSeq = new yaml.YAMLSeq();
		itemSeq.flow = true;
...
async function writeExtConfig(configPath, doc) {
	await (0, fs_promises.writeFile)(configPath, doc.toString({
		indent: 2,
		lineWidth: 0,
...
async function generateActionFiles(appManifest, actions, extensionPointId, templatesDir) {
	...
	consola.consola.success(`Generated ${actions.length} action(s)`);
	consola.consola.log.raw((0, consola_utils.formatTree)(outputFiles));
}
/**
...
function applyCustomScripts(installationTemplate, customScriptsTemplate) {
	...
	consola.consola.debug("No custom installation steps found, skipping custom-scripts.js generation...");
...
async function promptForCommerceAppConfig() {
	...
	const appName = await consola.default.prompt("What is the name of your app?", {
		type: "text",
		placeholder: "Application Display Name",
		default: "My Application",
		cancel: "reject"
	});
	const featuresToAdd = await consola.default.prompt(`What features do you want to add to your app? ${(0, consola_utils.colorize)("gray", "Space to select/deselect, Enter to submit")}`, {
		type: "multiselect",
		initial: [],
		required: true,
		cancel: "reject",
...
async function getDefaultCommerceAppConfig(cwd, { appName, configFormat, domains }) {
	...
	return [
		importStatement,
		"",
		`${exportKeyword} defineConfig(${require_logging.inspect(defaultConfig, { colors: false })})`
	].join("\n");
}
/**
...
async function addExtensionPointToAppConfig(extensionPointId, rootDirectory, commentBefore) {
	...
	if (doc.getIn([
		...
		consola.default.success(`Extension "${extensionPointId}" already configured in ${APP_CONFIG_FILE}`);
		return;
	}
	consola.default.info(`Adding extension "${extensionPointId}" to ${APP_CONFIG_FILE}...`);
	...
	await (0, fs_promises.writeFile)(appConfigPath, doc.toString(), "utf-8");
}
/**
...
async function addExtensionPointToInstallYaml(extensionPointId, rootDirectory, commentBefore) {
	const installYamlPath = (0, path.join)(rootDirectory, INSTALL_YAML_FILE);
	let doc;
	let extensions;
	try {
	...
	if (extensions.items.some((item) => (0, yaml.isMap)(item) && item.get("extensionPointId") === extensionPointId)) {
		consola.default.success(`Extension "${extensionPointId}" already configured in ${INSTALL_YAML_FILE}`);
		return;
	}
	consola.default.info(`Adding extension "${extensionPointId}" to ${INSTALL_YAML_FILE}...`);
	...
	await (0, fs_promises.writeFile)(installYamlPath, doc.toString(), "utf-8");
}
...
async function handleCommand(commandName, target, handlers) {
	function invalidTargetError(message) {
		consola.default.error(message);
		consola.default.info(`Available targets: ${Object.keys(handlers).join(" | ")}`);
		process.exit(1);
	}
...
async function main() {
	...
	try {
		...
		consola.default.error(require_error.stringifyError(error));
	}
}
main();
