	...
	try {
		...
		if (!result.ok) {
			switch (result.reason) {
				case "missing-env":
					consola.default.warn(".env not found — run `aio app use` to configure your workspace.");
					break;
				case "no-ims-context":
					consola.default.warn("No IMS context configured — run `aio login` to authenticate.");
					break;
