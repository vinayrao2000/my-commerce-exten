...
consola = require_encryption.__toESM(consola, 1);
let path = require("path");
path = require_encryption.__toESM(path, 1);
let fs = require("fs");
require("@adobe/aio-lib-core-config");
let _adobe_aio_lib_ims = require("@adobe/aio-lib-ims");
_adobe_aio_lib_ims = require_encryption.__toESM(_adobe_aio_lib_ims, 1);
require("dotenv");
...
async function run$1(envPath) {
	...
	if (!process.env[ENCRYPTION_KEY_ENV_VAR$1]) {
		consola.default.info("No encryption key found in .env file. Generating new key...");
		replaceEnvVar(envPath, ENCRYPTION_KEY_ENV_VAR$1, require_encryption.generateEncryptionKey());
		consola.default.success(`Encryption key generated and added to ${envPath}`);
		return;
	}
	consola.default.info("Encryption key already configured in .env file");
}
/** Run the encryption setup command */
async function exec$1() {
	try {
		const packageJson = await findNearestPackageJson();
		await run$1((0, path.join)(packageJson ? (0, path.dirname)(packageJson) : process.cwd(), ".env"));
	} catch (error) {
...
async function run(envPath) {
	const projectDir = (0, path.dirname)(envPath);
	process.loadEnvFile(envPath);
	...
	require_encryption.validateEncryptionKey(process.env[ENCRYPTION_KEY_ENV_VAR]);
	consola.default.success("Encryption key is valid");
}
/** Run the encryption validate command */
async function exec() {
	try {
		const packageJson = await findNearestPackageJson();
		await run((0, path.join)(packageJson ? (0, path.dirname)(packageJson) : process.cwd(), ".env"));
	} catch (error) {
		consola.default.error(error);
		process.exit(1);
	}
