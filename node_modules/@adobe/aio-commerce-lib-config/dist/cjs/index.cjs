...
const require_encryption = require('./encryption-BaPcaZWX.cjs');
let valibot = require("valibot");
valibot = require_encryption.__toESM(valibot, 1);
...
safe_stable_stringify = require_encryption.__toESM(safe_stable_stringify, 1);
let _adobe_aio_lib_core_logging = require("@adobe/aio-lib-core-logging");
_adobe_aio_lib_core_logging = require_encryption.__toESM(_adobe_aio_lib_core_logging, 1);
...
const CommerceEnvSchema = valibot.picklist(COMMERCE_ENVS, `Expected one of: ${COMMERCE_ENVS.map((e) => `"${e}"`).join(", ")}`);
/**
...
*/
const EnvSchema = valibot.pipe(valibot.array(CommerceEnvSchema, "Expected an array of commerce environments for the field \"env\""), valibot.nonEmpty("The \"env\" array must contain at least one commerce environment"));
/**
...
const BaseOptionSchema = valibot.object({
	...
	description: valibot.optional(valibot.pipe(valibot.string("Expected a string for the field description"), valibot.check(validateMarkdownLink, "Field description contains an invalid markdown URL."))),
	env: valibot.optional(EnvSchema)
});
...
const MultipleListSchema = valibot.object({
	...
	default: valibot.optional(valibot.array(valibot.pipe(valibot.string("Expected a string for each default value"), valibot.nonEmpty("Each default value must not be empty")), "Expected an array of default values"), DEFAULT_MULTIPLE_LIST_VALUE)
});
...
const TextSchema = valibot.object({
	...
	default: valibot.optional(valibot.string("Expected a string for the default value"), DEFAULT_STRING_VALUE)
});
...
const PasswordSchema = valibot.object({
	...
	default: valibot.optional(valibot.literal(DEFAULT_STRING_VALUE, "Password fields do not have a default value"), DEFAULT_STRING_VALUE)
});
...
const EmailSchema = valibot.object({
	...
	default: valibot.optional(valibot.union([valibot.literal(DEFAULT_STRING_VALUE), valibot.pipe(valibot.string("Expected a string for the default email value"), valibot.email("The email must be a valid email address"))]), DEFAULT_STRING_VALUE)
});
...
const UrlSchema = valibot.object({
	...
	default: valibot.optional(valibot.union([valibot.literal(DEFAULT_STRING_VALUE), valibot.pipe(valibot.string("Expected a string for the default URL value"), valibot.url("The URL must be a valid URL"))]), DEFAULT_STRING_VALUE)
});
...
const PhoneSchema = valibot.object({
	...
	default: valibot.optional(valibot.union([valibot.literal(DEFAULT_STRING_VALUE), valibot.pipe(valibot.string("Expected a string for the default phone number value"), valibot.regex(/^\+?[0-9\s\-()]+$/, "The phone number must contain only numbers and/or country codes"))]), DEFAULT_STRING_VALUE)
});
/** Schema for a boolean toggle field that accepts true/false values */
const BooleanSchema = valibot.object({
	...BaseOptionSchema.entries,
	type: valibot.literal("boolean", "Expected the type to be 'boolean'"),
	default: valibot.optional(valibot.boolean("Expected a boolean for the default value"), DEFAULT_BOOLEAN_VALUE)
});
/** Entries shared between the single- and multiple-selection dynamic list field schemas. */
const DynamicListEntriesCommon = {
	...BaseOptionSchema.entries,
	type: valibot.literal("dynamicList", "Expected the type to be 'dynamicList'"),
	options: valibot.custom((input) => typeof input === "function", "Expected a function for \"options\"")
};
/** Schema for a dynamic list field that allows single selection. */
const SingleDynamicListSchema = valibot.object({
	...DynamicListEntriesCommon,
	selectionMode: valibot.literal("single", "Expected the selectionMode to be 'single'"),
	default: valibot.custom((input) => typeof input === "function", "Expected a function for \"default\"")
});
/** Schema for a dynamic list field that allows multiple selections. */
const MultipleDynamicListSchema = valibot.object({
	...DynamicListEntriesCommon,
	selectionMode: valibot.literal("multiple", "Expected the selectionMode to be 'multiple'"),
	default: valibot.optional(valibot.custom((input) => typeof input === "function", "Expected a function for \"default\""))
});
/** Schema for dynamic list fields supporting either single or multiple selection modes. */
const DynamicListSchema = valibot.variant("selectionMode", [SingleDynamicListSchema, MultipleDynamicListSchema]);
...
async function getSharedState() {
	if (!__sharedState) {
		...
		__sharedState = await (0, _adobe_aio_lib_state.init)({ region: initRegion === "auto" ? getOptimalRegion(process.env.__OW_REGION) : initRegion });
	}
...
async function getSharedFiles() {
	if (!__sharedFiles) __sharedFiles = await (0, _adobe_aio_lib_files.init)();
...
function getLogger(moduleName) {
	return (0, _adobe_aio_lib_core_logging.default)(moduleName, { level: process.env.LOG_LEVEL ?? "info" });
}
...
async function setCachedScopeTree(namespace, data, ttlSeconds) {
	...
	try {
		await (await getSharedState()).put(`${namespace}:scope-tree`, (0, safe_stable_stringify.stringify)({ data }), { ttl: ttlSeconds });
	} catch (error) {
...
async function saveScopeTree(namespace, scopes) {
	...
	try {
		...
		await files.write(filePath, (0, safe_stable_stringify.stringify)(data, null, 2));
	} catch (error) {
...
*/
async function setCachedConfig(scopeCode, payload, ttlSeconds) {
	const logger = getLogger("@adobe/aio-commerce-lib-config:configuration-repository");
	try {
		const state = await getSharedState();
		const key = getConfigStateKey(scopeCode);
		await state.put(key, (0, safe_stable_stringify.default)({ data: payload }), { ttl: ttlSeconds });
	} catch (error) {
...
* @param ttlSeconds - Time to live for the cached configuration value.
*/
async function persistConfig(scopeCode, payload, ttlSeconds) {
	const payloadString = (0, safe_stable_stringify.default)(payload);
	const logger = getLogger("@adobe/aio-commerce-lib-config:configuration-repository");
...
* @returns Promise resolving to parsed configuration or null if not found.
*/
async function loadFromPersistedFiles(scopeCode, ttlSeconds) {
	const logger = getLogger("@adobe/aio-commerce-lib-config:configuration-repository");
	try {
...
*/
async function loadConfig(scopeCode, ttlSeconds) {
	...
	return {
		...
		config: configData.config.map((entry) => {
			if (passwordFields.has(entry.name) && typeof entry.value === "string" && entry.value.length > 0) {
				...
				return {
					...entry,
					value: require_encryption.decrypt(entry.value, encryptionKey)
				};
			}
			return entry;
		})
...
async function getConfigurationByKey$1(context, configKey, ...args) {
	...
	if (configValue && passwordFields.has(configKey) && typeof configValue.value === "string" && configValue.value.length > 0) {
		...
		configValue = {
			...configValue,
			value: require_encryption.decrypt(configValue.value, context.encryptionKey)
		};
	}
	return {
...
function encryptPasswordFields(entries, passwordFields, encryptionKey) {
	return entries.map((entry) => {
		if (entry.value !== null && passwordFields.has(entry.name) && typeof entry.value === "string" && entry.value.length > 0) {
			...
			return {
				...entry,
				value: require_encryption.encrypt(entry.value, encryptionKey)
			};
		}
		return entry;
	});
...
const logger = (0, _adobe_aio_lib_core_logging.default)("@adobe/aio-commerce-lib-config:get-scope-tree", { level: process.env.LOG_LEVEL ?? "info" });
...
function initializeCommerceClient(commerceConfig) {
	try {
		return new _adobe_aio_commerce_lib_api.AdobeCommerceHttpClient(commerceConfig);
	} catch (error) {
...
//#endregion
exports.SchemaBusinessConfig = SchemaBusinessConfig;
exports.byCode = byCode;
exports.byCodeAndLevel = byCodeAndLevel;
exports.byScopeId = byScopeId;
exports.byStoreId = byStoreId;
exports.byStoreViewId = byStoreViewId;
exports.byWebsiteId = byWebsiteId;
exports.generateEncryptionKey = require_encryption.generateEncryptionKey;
exports.getConfiguration = getConfiguration;
exports.getConfigurationByKey = getConfigurationByKey;
exports.getScopeTree = getScopeTree;
exports.hasDynamicSchema = hasDynamicSchema;
exports.initialize = initialize;
exports.resolveBusinessConfigSchema = resolveBusinessConfigSchema;
exports.setConfiguration = setConfiguration;
exports.setCustomScopeTree = setCustomScopeTree;
exports.syncCommerceScopes = syncCommerceScopes;
exports.unsyncCommerceScopes = unsyncCommerceScopes;
exports.validateEncryptionKey = require_encryption.validateEncryptionKey;