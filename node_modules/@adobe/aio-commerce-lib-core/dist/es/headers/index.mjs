...
import { t as CommerceSdkValidationError } from "../error-WnQqveay.mjs";
import { literal, nonEmpty, object, pipe, rawCheck, rawTransform, record, safeParse, string, trim, undefined as undefined$1, union } from "valibot";
...
function getHeadersFromParams(params) {
	const parsed = safeParse(object({ __ow_headers: record(string(), union([string(), undefined$1()])) }), params);
	if (!parsed.success) throw new CommerceSdkValidationError("Missing __ow_headers in action params", { issues: parsed.issues });
...
function getMissingOrEmptyHeaders(headers, requiredHeaders) {
	...
	const parsed = safeParse(buildHeadersSchema(requiredHeaders.map((h) => literal(h))), normalized);
...
function buildHeadersSchema(requiredHeaders) {
	const buildSchema = () => pipe(string("Expected a string value for the header"), rawCheck(({ dataset, addIssue }) => {
...
function assertRequiredHeadersSchema(headers, requiredHeaders) {
	...
	const parsed = safeParse(buildHeadersSchema(requiredHeaders), normalized);
	if (!parsed.success) throw new CommerceSdkValidationError("The headers object does not match the required headers (case-insensitive)", { issues: parsed.issues });
}
/**
...
function assertRequiredHeaders(headers, requiredHeaders) {
	assertRequiredHeadersSchema(headers, requiredHeaders.map((header) => literal(header)));
}
...
const authorizationSchema = pipe(string("Expected a string value"), trim(), nonEmpty("Expected a non-empty string value"), rawTransform(({ dataset: { value: authorization }, addIssue, NEVER }) => {
...
function parseAuthorization(authorization) {
	...
	if (!parsed.success) throw new CommerceSdkValidationError("Invalid Authorization header format", { issues: parsed.issues });
...
const bearerSchema = object({ scheme: literal("Bearer") });
/**
...
function parseBearerToken(authorization) {
	...
	if (!parsed.success) throw new CommerceSdkValidationError("Authorization header must be a Bearer token", { issues: parsed.issues });
	return auth;
}
const basicSchema = object({ scheme: literal("Basic") });
/**
...
function parseBasicToken(authorization) {
	...
	if (!parsed.success) throw new CommerceSdkValidationError("Authorization header must be Basic authentication", { issues: parsed.issues });
	return auth;
}
const oauthSchema = object({ scheme: literal("OAuth") });
/**
...
function parseOAuthToken(authorization) {
	...
	if (!parsed.success) throw new CommerceSdkValidationError("Authorization header must be OAuth", { issues: parsed.issues });
	return auth;
}

//#endregion
export { assertRequiredHeaders, createHeaderAccessor, getHeader, getHeadersFromParams, getMissingOrEmptyHeaders, isBasicAuth, isBearerAuth, isOAuth, parseAuthorization, parseBasicToken, parseBearerToken, parseOAuthToken };